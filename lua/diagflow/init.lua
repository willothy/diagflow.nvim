...
function M.setup(user_config)
	...
	if type(config.severity_colors) ~= "table" then
		error(
			'diagflow: Invalid type for "severity_colors" config. Expected table, got ' .. type(config.severity_colors)
		)
		return
	end
	...
	if
		...
		or (config.scope ~= "line" and config.scope ~= "cursor" and config.scope ~= "node")
	then
	...
	if type(config.inline_padding_left) ~= "number" then
		error(
			'diagflow: Invalid type for "inline_padding_left" config. Expected number, got '
