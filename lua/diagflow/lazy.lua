...
function M.init(config)
	...
	local function render_diagnostics()
		...
		if config.scope == "node" then
			...
			node_range = { vim.treesitter.get_node_range(node) }
		end
		...
		for _, diag in ipairs(diags) do
			if
				(
					config.scope == "node"
					and node_range ~= nil
					and diag.lnum >= node_range[1]
					and (diag.end_lnum <= node_range[3])
				)
				or (config.scope == "line" and diag.lnum == line)
				or (
					config.scope == "cursor"
					and diag.lnum == line
					and diag.col <= col
					and (diag.end_col or diag.col) >= col
				)
			then
...
function M.clear()
	pcall(function()
		vim.api.nvim_del_augroup_by_name("RenderDiagnostics")
	end)
	pcall(function()
		vim.api.nvim_buf_clear_namespace(0, ns, 0, -1)
	end)
